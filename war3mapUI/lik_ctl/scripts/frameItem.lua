...
function likCtl_frameItem(kit, stage)
    ...
    stage.itemWeight = FrameText(kitIt .. '->weight', stage.item)
        :relation(FRAME_ALIGN_BOTTOM, stage.item, FRAME_ALIGN_TOP, -0.01, -0.013)
        :textAlign(TEXT_ALIGN_CENTER)
        :fontSize(9)
        :size(0.08, 0.008)
        :onEvent(EVENT.Frame.Leave, function(_) FrameTooltips():show(false, 0.4) end)
        ...
        function()
            ...
            FrameTooltips()
                ...
                :relation(FRAME_ALIGN_BOTTOM, stage.itemWeight, FRAME_ALIGN_TOP, 0, 0.002)
                :content({ tips = { "负重" } })
                :show(true)
        end)
            ...
            function(evtData)
                ...
                if (content ~= nil) then
                    FrameTooltips()
                        ...
                        :fontSize(10)
                        :textAlign(TEXT_ALIGN_LEFT)
                        :relation(FRAME_ALIGN_BOTTOM, stage.itemButton[i], FRAME_ALIGN_TOP, 0, 0.001)
                        :content(content)
                        :show(true)
                end
            end)
